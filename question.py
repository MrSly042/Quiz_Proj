...
base = Tk()
base.title("FIRS PRACTICE QUIZ") #program window title
base.attributes("-fullscreen", True) #set the definitions of root window
screen_height = base.winfo_screenheight()
screen_width = base.winfo_screenwidth()
# base.geometry("%dx%d+0+0" % (screen_width, screen_height))

# root window icon
icon = ImageTk.PhotoImage(file = 'logo.jpg')
base.iconphoto(False, icon)
base.resizable(height=0, width=0)
base.protocol("WM_DELETE_WINDOW", lambda: 'break')
...
sub_img = PhotoImage(file = "sub.png")
...
output = [] # list to store values "WRONG" or "CORRECT" based on choice
...
dice = {} #dict to store user choices based on screen index as key and values as values
...
def commence():
    ...
    def update_timer(seconds_left):
            ...
            if seconds_left > 0:
                update_aft = base.after(1000, update_timer, seconds_left - 1)
    
    main_cont = Frame(base)
    main_cont.pack(fill=BOTH, expand=True)
    
    main_canv = Canvas(main_cont, )
    main_canv.pack(side=LEFT, fill=BOTH, expand=True)
    
    main_scroll_horiz = Scrollbar(base, orient='horizontal',
                                  command=main_canv.xview,
                                  )
    
    main_scroll_horiz.pack(side=BOTTOM, fill=X)
    
    main_scroll_vert = Scrollbar(main_cont, command=main_canv.yview,
                                )
    ...
    main_cont_canv = Frame(main_canv, )
    ...
    holder = Frame(main_cont_canv)
    holder.pack(side=LEFT, fill=BOTH, expand=True, 
                )
    
    tools = ttk.Frame(holder)
    ...
    timer_label = Label(tools, text='1hr 00mins 00secs', font=('Arial', 24))
    ...
    # changes minibar colours based on whether screen have chosen options in them
    ...
    quest_frame = Frame(holder)
    question_itself = Frame(quest_frame)
    choice_frame = Frame(quest_frame)
    question_label = Label(question_itself, text='')

    v = StringVar(choice_frame, 5)

    # declaring radiobtns using for loop
    # each call "select_ans func() using respaecting index"
    choice_radiobuttons = [Radiobutton(choice_frame,value=i, variable=v,command = lambda v = i: select_answer(v)) for i in range(3)]

    nav_frame = Frame(holder)
    previous_button = Button(nav_frame, image=prev_img, command=previous_question)
    next_button = Button(nav_frame, image=next_img, command=next_question)
    
    mini_bar = Frame(holder)
    mini_bar_buttons = []
    for i in range(len(questions)):
        button = Button(mini_bar, text=i+1, command=lambda idx=i: mini_func(idx), 
                        bg='red', fg="white")
...
def try_close():
    ...
    show_res = Frame(base, bg='black')
    show_res.pack(side=TOP, fill=BOTH, expand=True)
    
    #canvas for results screen
    show_canv = Canvas(show_res, )
    show_canv.pack(side=LEFT, fill=BOTH, expand=True)
    
    #scrollbars inside canvas
    show_scroll_horiz = Scrollbar(base, orient='horizontal', command=show_canv.xview)
    show_scroll_horiz.pack(side=BOTTOM, fill=X)
    
    show_scroll_vert = Scrollbar(show_res, command=show_canv.yview)
    ...
    result_frame = Frame(cont_frame, bg = 'black')
    base.config(bg='black')
    score_frame = Frame(result_frame, bg = 'black')
    exit_frame = Frame(result_frame, bg = 'black')
    others_frame = Frame(result_frame, bg = 'black' )
    
    exit_btn = ttk.Button(exit_frame,image=exit_img, command=base.destroy,
                   )
    
    score_sheet = Label(score_frame,text = "YOU SCORED: \n" +
                        "{}/{}".format(good, total), bg='sky blue',  font = ("Times New Roman", 18, 'bold'))
    
    other1 = Label(others_frame, bg='orange',relief='sunken', bd=10,
                   font = ("Times New Roman", 18, 'bold'),)
    
    other2 = Button(others_frame, text="SEE CORRECTIONS", bd=10,
...
def corrections():
    ...
    #new frame for results screen      
    corr_frame = Frame(base, )
    corr_frame.pack(side=TOP, fill=BOTH, expand=True)
    
    #canvas for results screen
    corr_canv = Canvas(corr_frame, bg = 'light green')
    corr_canv.pack(side=LEFT, fill=BOTH, expand=True)
    
    #scrollbars inside canvas
    corr_scroll_horiz = Scrollbar(base, orient='horizontal', command=corr_canv.xview)
    corr_scroll_horiz.pack(side=BOTTOM, fill=X)
    
    corr_scroll_vert = Scrollbar(corr_frame, command=corr_canv.yview)
    ...
    correct_me = Label(new, text=show_every, bg='light green', font = ("Comic sans ms", 15, 'bold'),
                       justify='left')
...
def helping():
    ...
    dis_frame = Frame(dis, background = 'cyan')
    dis_frame.grid(row=0, column=0)
    #should include icon image
    info = Label(dis_frame,text = instructions, justify='left',
...
def show_inst():
            ...
            contain_canv_instruct = Frame(base)
            contain_canv_instruct.pack(fill=BOTH, expand=True)
            
            #canvas widget
            instruct_canvas = Canvas(contain_canv_instruct, bg = 'sky blue')
            instruct_canvas.pack(side=LEFT, fill=BOTH, expand=True)
            
            #horizontal scrollbar
            inst_scroll_horiz = Scrollbar(instruct_canvas, orient='horizontal', 
            ...
            #vertical scrollbar
            inst_scroll_vert = Scrollbar(contain_canv_instruct, command=instruct_canvas.yview,
                                        )
            ...
            instruct_cont_canv = Frame(instruct_canvas, bg="sky blue")
            ...
            new = Frame(instruct_cont_canv, bg="sky blue", )
            new.grid(row=0, column=0, )
                                    
            #instructions header
            page2_head = Label(new, text = "INSTRUCTIONS", background="black",
                                foreground="white", font = ("comic sans ms", 40, 'bold'))
            page2_head.grid(row = 0, column = 0,
                            ipadx=580, ipady=50)
            #info
            information = Label(new, text = instructions, justify = "left",
                                background = "sky blue",font=("Calibri", 15, 'bold')
                                )
            information.grid(row=1, column=0, sticky = "W")
            
            #extra info(warnings)
            warn = Label(new, text = consequence, foreground = 'red',
                          background = "sky blue",font=("Calibri", 15, "bold"),
                         justify = "left")
...
def get_input():
    ...
    if user == 'Gates' and pass_w == '123456':
...
container = Frame(base, bg='violet')
container.pack(side=TOP,fill=BOTH, expand=True)

#canvas for scrollbar
canvase = Canvas(container, bg='violet')
canvase.pack(side=LEFT, fill=BOTH, expand=True)
#scrollbar for login page
login_scroll_horiz = Scrollbar(canvase, orient='horizontal',command=canvase.xview,
                               )
login_scroll_horiz.pack(side=BOTTOM, fill=X)

login_scroll_vert = Scrollbar(container, command=canvase.yview)
...
cont_within_base = Frame(canvase, bg='violet')
#create window
canvase.create_window((0,0), window = cont_within_base, 
                      anchor='nw', 
                      )


#login section, nested in master frame above
header = Frame(cont_within_base, background='purple')
header.grid(row=0, column=0,)

login_page = Label(header, background='purple', foreground='white',
...
frame = Frame(cont_within_base, bg='violet')
frame.grid(row=1, column=0, sticky='w')

#entry segments for credentials
username = Label(frame, text='Username:', foreground='red',
                font=('calibri', 30, 'normal'), background='violet')

password = Label(frame, text = 'Password:', foreground='red',
                 font=('Calibri', 30, 'normal'), background='violet')
...
user_entry = Entry(frame, textvariable=user_value,
                   font=('Comic Sans', 15), width=50
                   )

pass_entry = Entry(frame, textvariable=Pass_value,
...
cont_within_base.bind("<Configure>", change)
